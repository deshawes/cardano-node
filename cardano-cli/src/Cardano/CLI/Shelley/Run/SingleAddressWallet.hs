...
import           Cardano.Api (Address (..), Lovelace (..), Network (..),
                     PaymentVerificationKey (..), SigningKey (..), TxId (..), TxIn (..),
                     TxOut (..), TxSigned (..), TxUnsigned (..), buildDummyShelleyTxForFeeCalc,
                     buildShelleyTransaction,calculateShelleyMinFee, getLocalTip,
...
buildSingleAddressWalletTransaction codecCfg network sockPath mbTxTtl
    ...
    utxo <- firstExceptT NodeLocalStateQueryError $
      queryFilteredUTxOFromLocalState network sockPath (Set.singleton addr) tipPoint
